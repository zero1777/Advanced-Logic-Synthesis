...
private:
    ...
    int MAX_VERIF_ITER;
    sat_n::Solver Scand;             // this solver produces candidates.
    sat_n::Solver Scex;              // "doubled-ckt" solver.
    AllSAT::ClauseList cl;           // Scex solver with the initial list of clauses.
    AllSAT::ClauseList candidate_cl; // Scand solver with the initial list of clauses.

    ckt_n::index2lit_map_t lmap;  // map for Scex
    ckt_n::index2lit_map_t candidate_lmap; // map for Scand.
    std::map<sat_n::Lit, sat_n::Lit> Scand_to_Scex_map;

    // dbl_keyinput_flags[var(l)] tells us whether l is a key input
    std::vector<bool>           dbl_keyinput_flags;
    // ckt_keyinput_flags[var(l)] tells us whether l is a key input
    std::vector<bool>           ckt_keyinput_flags;
    ...
    // literals for the candidate solver.
    std::vector<sat_n::Lit> candidate_keyinput_literals;
    std::vector<int> candidate_keyinputs;
    std::vector<sat_n::Lit> candidate_cktinput_literals;
    std::vector<sat_n::Lit> candidate_output_literals;
    sat_n::vec_lit_t cex_assumptions;
    sat_n::vec_lit_t cand_assumptions;
    ...
    unsigned Scand_nProblemVars, Scex_nProblemVars;

    // appsat data structures.
    ckt_n::enc_ckt_eval_t appsat_sim;
    std::vector<bool> key_values;
    std::vector<bool> appsat_outputs;
    std::mt19937 mersenne_twister;
    std::uniform_int_distribution<> bit_generator;
    int appsat_settlement_count;

    // appsat methods.
    int _appsat_iteration();
    void _appsat_set_sim_keys();
    bool _appsat_random_query();

    // methods.
    void _sanity_check_model();
    int _verify_solution_sim(std::map< std::string, int >& keysFound);
    ...
    bool _get_distinguishing_input();
