...
// appsat options.
int appsat_iter = 0;
int appsat_queries = 50;
int appsat_threshold = 5;

std::string cnf_dump_prefix;
...
int sld_main(int argc, char* argv[]) 
{
    ...
    while ((c = getopt (argc, argv, "ihvptTc:m:k:sN:D:a:q:S:")) != -1) {
        switch (c) {
            case 'h':
            ...
            case 'N':
                ...
                break;
            case 'D':
                cnf_dump_prefix = optarg;
                break;
            case 'a':
                appsat_iter = atoi(optarg);
                break;
            case 'q':
                appsat_queries = atoi(optarg);
                break;
            case 'S':
...
int print_usage(const char* progname)
{
    std::cout << "Usage: " << progname << " [options] <encrypted-bench-file> <original-bench-file>" 
              << std::endl << std::endl;
    std::cout << "  Process control options." << std::endl;
    ...
    std::cout << "    -N            : extract N different keys (default=1)." << std::endl;
    ...
    std::cout << "    -a            : iterations between sampling (default=0 means AppSAT disabled)." << std::endl;
    std::cout << "    -q            : # of sampling queries (default=50)." << std::endl;
    std::cout << "    -S            : settlement threshold (default=5)." << std::endl;
