...
namespace ckt_n {
    ...
    int ckt_t::count_differing_inputs(const char* keys, int MAX_CNT)
    {
        ...
        _solver_set_keys(S, map, keys);

        Lit lout = getLit(map, outputs[0]);
        S.addClause(lout);

        for(unsigned i=0; i != num_ckt_inputs(); i++) {
            Lit li = getLit(map, ckt_inputs[i]);
            S.freeze(li);
        }

        int cnt = 0;
        while (S.solve() && cnt < MAX_CNT) {
            vec_lit_t blocking_clause;
            for (unsigned i=0; i != num_ckt_inputs(); i++) {
                Lit li = getLit(map, ckt_inputs[i]);
                lbool vi = S.modelValue(li);
                if (vi.isDef()) {
    ...
    void ckt_t::_solver_set_keys(sat_n::Solver& S, index2lit_map_t& map, const char* keys) {
        using namespace sat_n;
